...
public class SwitchSimTemplateUtils {
	...
	private static final String propFileUrl = "src/main/resources/"+SwitchSimController.propMibFile;
	...
	public String generateMib(Switch device) {
		...
		return render("mib.ftl", ImmutableMap.builder()
                .put("ports", ports)
                .build());
	}
	
	public void storeMib(Switch device) {
		Properties prop = new Properties();
		OutputStream output = null;

		try {
			output = new FileOutputStream(propFileUrl);
			...
			for(Card card : cards) {
				for(Port port : card.getPorts()) {
					prop.setProperty(".1.3.6.1.2.1.2.2.1.2."+port.getPortID(), "STRING: 10/100 utp ethernet (cat 3/5)");
				}
			}
			...
			for(Card card : cards) {
				for(Port port : card.getPorts()) {
					prop.setProperty(".1.3.6.1.2.1.2.2.1.9."+port.getPortID(), "Timeticks: (1620595854) 187 days, 13:39:18.54");
				}
			}
			// save properties to project root folder
			prop.store(output, null);
			//the colon is escaped with a back slash so we just need to overwrite it
			PrintWriter pw = new PrintWriter(propFileUrl); 
